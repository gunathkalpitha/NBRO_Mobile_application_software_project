...
RETURNS BOOLEAN AS $$
BEGIN
    RETURN (SELECT role = 'admin' AND is_active = true FROM public.profiles WHERE id = auth.uid());
END;
...
-- Inspections policy - admins
CREATE POLICY "inspections_admin_all" ON inspections FOR ALL USING (public.is_admin());
