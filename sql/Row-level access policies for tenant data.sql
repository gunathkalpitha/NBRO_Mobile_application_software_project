-- Drop all existing policies on app tables so this script can be re-run safely
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN
    ...
    AND tablename IN (
      'profile', 'site', 'general_observation', 'external_services',
      'ancillary_building', 'detail_type', 'building_detail', 'main_building',
      'specification', 'defects', 'defect_info', 'defect_image'
    )
  LOOP
